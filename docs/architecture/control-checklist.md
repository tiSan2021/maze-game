...
- [ ] **[AUTO] 三必做前置已声明为不可省略项**（缺一则关 9+ 9–11ms 超标）：
      ...
      2. 视野半径 **R ≤ 3**（当前值 R=2；`metrics.ts` 中 `VISION_R === 2` 单元断言，仅禁止改大至 4/5）。
...
- [ ] **[AUTO] 性能三前置的 CI 代理断言**（真机实测无法在 CI 判定，故设代理指标拦截回归）：
      ...
      2. `metrics.VISION_R === 2` 常量断言（仅禁止改大至 4/5；缩小如 3→2 不在禁止范围），且代码中无 R=4/5 分支；
...
- [ ] **[AUTO] R 上限为 3（当前值 2）**：`VISION_R` 当前值 2（2026-09-09 由 3 缩至 2，难度提升），不可由配置改大至 4/5；R=4/5 不在任何代码路径（M5）。
