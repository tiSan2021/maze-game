...
// ADR-03 F 组：VISION_R 恒定为 3（M5 不可由配置改大）；关 9+ 性能预算 <8ms 的三前置之一。
// 注意：VISION_R 是 `const`，全仓无任何代码路径能把它改大到 4/5（控制清单 F 组 R 恒定为 3）。
...
/**
 * 视野半径（切比雪夫距离）。全程恒定 = 3（D12 纯半径、不做 Bresenham）。
 * 这是 M5 的硬约束落点：任何更大的视野半径分支都不允许存在。
 */
export const VISION_R = 3;
